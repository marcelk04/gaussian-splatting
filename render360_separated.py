...
def render_sets(dataset1 : ModelParams, dataset2 : ModelParams, iteration : int, pipeline : PipelineParams, separate_sh: bool, output: str, shs_idx=0):
	with torch.no_grad():
		...
		if output == "":
			render_path = os.path.join(dataset1.model_path, "test", "ours_{}".format(scene.loaded_iter), "video")
		...
		cams = scene.getTrainCameras1()

		w, h = Image.open(os.path.join(dataset1.source_path, "images", "0000.png")).size
		resolution = (h, w)
		FoVx = cams[0].FoVx
		FoVy = cams[0].FoVy
		depth_params = None
...
if __name__ == "__main__":
	...
	render_sets(dataset1, dataset2, args.iteration, pipeline.extract(args), SPARSE_ADAM_AVAILABLE, args.output, args.shs_idx)